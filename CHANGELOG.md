...
- replace `isPageHeading: true` with `heading: true`
...
You will not need to set the label or legend `headingLevel` option unless it acts as the heading for a landmark. For example, using an `<h2>` for a search input label in an `<aside>` element:
